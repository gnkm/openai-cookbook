...
OpenAI Cookbookのこのセクションでは、セマンティック検索のユースケースをサポートするために利用可能な多くのベクターデータベースを紹介しています。

ベクターデータベースは、知識検索アプリケーションにとって優れた補完ツールとなり得ます。これらは、質問に答えるための関連するコンテキストをLLMに提供することで、ハルシネーション（幻覚）を減らすことができます。

各プロバイダーには独自の名前付きディレクトリがあり、私たちのAPIと彼らの製品を使用する方法を紹介する標準的なノートブック、および彼らが機能を紹介するために追加することを選択した補足的なノートブックが含まれています。
